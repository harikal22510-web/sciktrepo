"""
Enhanced Acquisition Functions - File 3
Sophisticated acquisition functions for complex optimization scenarios.
"""
...
class AcquisitionFunctionComparator:
    """
    Comparator for different acquisition functions.
    
    This class allows systematic comparison of multiple acquisition functions
    on the same optimization problem.
    """
    
    def __init__(self, acquisition_functions=None):
        """
        Initialize acquisition function comparator.
        
        Parameters
        ----------
        acquisition_functions : list
            List of acquisition function names to compare.
        """
        if acquisition_functions is None:
            acquisition_functions = ['ei_plus', 'pi_plus', 'lcb_plus', 'kg', 'ts', 'mes']
        
        self.acquisition_functions = acquisition_functions
        self.results_ = {}
        
    def compare(self, X, model, y_opt=None):
        """
        Compare acquisition functions on the same data.
        
        Parameters
        ----------
        X : array-like
            Points to evaluate acquisition functions.
        model : object
            Fitted surrogate model.
        y_opt : float
            Current best objective value.
        ...
        -------
        results : dict
            Comparison results for each acquisition function.
        """
        results = {}
        
        for acq_name in self.acquisition_functions:
            try:
                acq_func = select_acquisition_function(acq_name)
                acq_values = acq_func(X, model, y_opt)
    ...
    def get_best_function(self, metric='max'):
        """
        Get the best acquisition function based on a metric.
        
        Parameters
        ----------
        metric : str
            Metric to use for comparison: 'max', 'mean', 'std'
        
        Returns
        -------
        best_func : str
            Name of the best acquisition function.
        """
        ...
        for acq_name, result in self.results_.items():
            if 'error' not in result and metric in result:
                ...
                if metric == 'std':
                    ...
                    # For max and mean, higher is better
                    if value > best_value:
                        best_value = value
                        best_func = acq_name
    ...
    def plot_comparison(self, X, save_path=None):
        """
        ...
        for i, (acq_name, result) in enumerate(self.results_.items()):
            if i >= len(axes):
                break
            ...
            if 'error' not in result:
                ax.plot(X[:, 0], result['values'], label=acq_name)
                ax.set_title(f'{acq_name}')
                ax.set_xlabel('X[:, 0]')
